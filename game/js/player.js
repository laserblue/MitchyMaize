...
/*
  this.direction = 'down';
  this.animations.add('down', [0, 1, 2, 3, 4, 5, 6, 7], 1, true);
  this.animations.add('up', [8, 9, 10, 11, 12, 13, 14, 15], 1, true);
  this.animations.add('right', [24, 25, 26, 27, 28, 29, 30, 31], 1, true);
  this.animations.add('left', [16, 17, 18, 19, 20, 21, 22, 23], 1, true);
*/
};
...
Player.prototype.reset = function(tilex,tiley) {
  ...
  //   this.game.add.existing(this);
     ...
     this.direction = 'down';
     this.animations.add('down', [6, 7], 2, true);
     this.animations.add('up', [8, 9], 2, true);
     this.animations.add('right', [4, 11], 2, true);
     this.animations.add('left', [5, 10], 2, true);
...
Player.prototype.movements = function() {
   var cc = this.cursors;
  if (!this.tweening) {
    if ( (cc.left.isDown || aKey.isDown || leftArrow)) {
        this.moveTo(-1,0);
        this.direction = 'left';
        this.animations.play('left');
      }
      else if ( (cc.right.isDown || dKey.isDown || rightArrow)) {
        this.moveTo(1,0);
        this.direction = 'right';
        this.animations.play('right');
      }
      else if ( (cc.up.isDown || wKey.isDown || upArrow)) {
        this.moveTo(0,-1);
        this.direction = 'up';
        this.animations.play('up');
      }
      else if ( (cc.down.isDown || sKey.isDown || downArrow)) {
        this.moveTo(0,1);
        this.direction = 'down';
...
Player.prototype.moveTo = function(x,y) {
  ...
  // console.log('cant move' + this.cantMove(x,y));
...
Player.prototype.cantMove = function(x,y) {
  ...
  if (this.map.getTile(newx, newy, 'layer1').collideDown) {
    return true;
  }

  //Block Layer 2 Collisions if applicable
  if (this.map.getTile(newx, newy, 'layer2') !== null) {
    return this.map.getTile(newx, newy, 'layer2').collideDown;
  }
  return false;
